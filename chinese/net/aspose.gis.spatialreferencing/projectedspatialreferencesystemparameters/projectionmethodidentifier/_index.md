...
description: "ProjectedSpatialReferenceSystemParameters 属性。投影方法的标识符。没有默认值，如果您想将标识符附加到投影，可以将此参数设置为非空值。如果这样做，您需要确保该标识符在一致的投影方法名称中保持一致，设置此属性时投影方法名称不会改变。"
type: docs
weight: 60
...
投影方法的标识符。没有默认值，您可以将此参数设置为非 `null` 值，如果您想为投影附加标识符。若这样做——您需确保标识符与投影方法名称保持一致（设置此属性时投影方法名称不会改变）。
