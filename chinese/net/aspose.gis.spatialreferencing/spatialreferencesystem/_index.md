...
description: 空间参考系统将坐标映射到地球上的位置 有不同类型的 SRS请参阅Type./spatialreferencesystem/type. 更重要的是如果 SRS 的类型是Geographicor Projected SRS 可以是复合的或单一的见IsCompound./spatialreferencesystem/iscompound.
type: docs
weight: 2150
...
空间参考系统将坐标映射到地球上的位置。 有不同类型的 SRS，请参阅[`Type`](./type). 更重要的是，如果 SRS 的类型是Geographicor Projected SRS 可以是复合的或单一的，见[`IsCompound`](./iscompound).
...
| [IsValid](../../aspose.gis.spatialreferencing/spatialreferencesystem/isvalid) { get; } | 同[`Validate`](./validate)，但不返回错误信息。 |
...
| static [CreateFromWkt](../../aspose.gis.spatialreferencing/spatialreferencesystem/createfromwkt)(string) | 创建一个新的`空间参考系统`基于 WKT（知名文本）字符串. |
...
| static [IsEquivalent](../../aspose.gis.spatialreferencing/spatialreferencesystem/isequivalent)(SpatialReferenceSystem, SpatialReferenceSystem) | 判断两个 SRS 是否等价。 等价 SRS 的相同坐标匹配地球上的同一个地方。 等价 SRS 的某些参数可以不同，例如[`Name`](../identifiableobject/name). |
...
| static [TryCreateFromWkt](../../aspose.gis.spatialreferencing/spatialreferencesystem/trycreatefromwkt)(string, out SpatialReferenceSystem) | 创建一个新的`空间参考系统`基于 WKT（知名文本）字符串. |
