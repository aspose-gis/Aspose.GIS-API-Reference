...
description: 确定两个基准是否等价 等效基准的相同坐标匹配地球上的相同位置 等效基准的某些参数可以不同例如Nameaspose.gis.spatialreferencing/identifiableobject/name
type: docs
weight: 90
...
确定两个基准是否等价。 等效基准的相同坐标匹配地球上的相同位置。 等效基准的某些参数可以不同，例如[`Name`](../../identifiableobject/name)。
