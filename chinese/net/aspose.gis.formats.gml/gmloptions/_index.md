...
| [LoadSchemasFromInternet](../../aspose.gis.formats.gml/gmloptions/loadschemasfrominternet/) { get; set; } | 确定是否允许 Aspose.GIS 从互联网加载 XML 模式。如果设置为 `false`，则不会加载以非 'file://' 开头的绝对 URI 的模式。默认是 `false`。 |
...
| [RestoreSchema](../../aspose.gis.formats.gml/gmloptions/restoreschema/) { get; set; } | 确定是否允许 Aspose.GIS 解析在缺少或无法加载 XML 模式的 Gml 文件中的属性。如果设置为 `true`，Aspose.GIS 读取器不需要 XML 模式的存在。默认是 `false`。 |
| [SchemaLocation](../../aspose.gis.formats.gml/gmloptions/schemalocation/) { get; set; } | 以空格分隔的 URI 对列表。每对中的第一个 URI 是命名空间的 URI，第二个 URI 是该命名空间的 XML 模式路径。如果设置为 `null`，[`GmlDriver`](../gmldriver/) 将尝试从文档根元素读取 schemaLocation。默认是 `null`。 |
