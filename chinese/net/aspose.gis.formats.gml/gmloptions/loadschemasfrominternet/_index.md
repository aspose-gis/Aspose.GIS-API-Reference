...
description: "GmlOptions 属性。确定是否允许 Aspose.GIS 从 Internet 加载 XML 架构。如果设置为 false，则具有以 file// 开头的绝对 URI 的架构将不会被加载。默认值为 false。"
type: docs
weight: 20
...
确定是否允许 Aspose.GIS 从互联网加载 XML 模式。如果设置为 `false`，则不会加载以非 'file://' 开头的绝对 URI 的模式。默认是 `false`。
