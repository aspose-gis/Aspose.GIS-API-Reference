...
description: 确定是否允许 Aspose.GIS 从 Internet 加载 XML 模式 如果设置为false具有不以file//开头的绝对 URI 的架构将不会被加载 默认为false.
type: docs
weight: 20
...
确定是否允许 Aspose.GIS 从 Internet 加载 XML 模式。 如果设置为`false`，具有不以“file://”开头的绝对 URI 的架构将不会被加载。 默认为`false`.
