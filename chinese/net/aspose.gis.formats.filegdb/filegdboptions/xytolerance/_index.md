...
这是一个创建选项，不影响读取和编辑。此参数控制用于高级 ArcGIS 功能的捕捉容差。它不影响 Aspose.GIS 的行为，但可被 ArcGIS 使用。参数的单位与空间参考系统的单位相同。如果设置为 `null`，则使用默认值。默认值取决于空间参考系统，对于地理坐标系统为 0.000000008983153 度，对于投影坐标系统为 0.001 米（值会转换为空间参考系统的单位）。如果空间参考系统未知，默认值为 0.001。
