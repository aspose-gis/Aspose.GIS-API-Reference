...
description: "Feature 方法。为所有属性设置新值。还可以考虑使用 CopyValues 方法，以在一次调用中简化设置值的过程"
type: docs
weight: 120
...
为所有属性设置新值。还可以考虑使用 [`CopyValues`](../copyvalues/) 方法，以在一次调用中简化设置值的过程。
...
此方法会自动将每个值转换为属性的类型。值数组的长度不必与要素中的属性数量匹配。如果数组长度大于属性数量，则所有数组值都会复制到属性中；如果数组长度较短，则仅复制数组长度对应数量的值到属性中，起始于序号为 0 的属性值。
