...
特征的值属性被复制到作为参数传入的 values 数组中。对于未设置值的属性，返回指定的 'unsetValue' 参数。values 数组的长度不必与特征中的属性数量匹配。如果数组长度大于属性数量，所有属性值都会复制到数组中；如果数组长度较小，则仅复制数组长度数量的属性值到数组中，起始于序号为 0 的属性值。
