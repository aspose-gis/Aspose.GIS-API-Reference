...
description: "VectorLayer 方法。加载属性索引以加快在诸如 WhereGreater 等过滤方法中按属性值进行过滤的速度。如果索引不存在，则先创建。使用 forceRebuild 强制重新创建索引"
type: docs
weight: 200
...
加载属性索引以加快在诸如 [`WhereGreater`](../../featuressequence/wheregreater/) 等过滤方法中按属性值进行过滤的速度。如果索引不存在，则先创建。使用 *forceRebuild* 强制重新创建索引。
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| indexPath | 字符串 | 索引文件的路径。 |
| attributeName | 字符串 | 用于构建索引的属性名称。 |
| forceRebuild | Boolean | 即使索引已存在，是否仍重新创建。 |
