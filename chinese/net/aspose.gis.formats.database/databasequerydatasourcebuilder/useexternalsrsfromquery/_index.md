...
description: "DatabaseQueryDataSourceBuilder 方法。允许您配置数据源使用第三方空间参考系统数据，绕过 Aspose.GIS 库中预装的数据"
type: docs
weight: 60
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| srsQuery | String | 用于检索主查询中用于获取要素的附加空间坐标系统信息的查询。 |
...
需要注意的是，如果在预先从数据库提取的空间参考系统列表中缺少主查询中使用的系统，库将尝试使用内置的 srs。查询示例：SELECT auth_srid, srtext FROM spatial_ref_sys WHERE srid = 4284
