...
此方法根据 DE-9IM 相交矩阵测试一个几何是否覆盖另一个几何。 一个几何是否覆盖另一个几何，如果该几何包含另一个几何的每个点。 此方法类似于[`SpatiallyContains`](../../igeometry/spatiallycontains) 但返回`true`更常见的是， ，因为它不区分内部点和边界点。因此，如果几何 A 位于 几何 B 的边界上，[`SpatiallyContains`](../../igeometry/spatiallycontains)返回`false`, 而这个方法返回`true`. 这个方法等价于：
