...
description: GeometryCollection 方法. 使用默认值获取此几何的近似或等效非曲线版本宽容.
type: docs
weight: 220
...
没有曲线几何的几何。这相当于[`ToLinearGeometry`](../../igeometrycollection/tolineargeometry/)with 默认值`宽容`.默认`宽容` 值取决于[`SpatialReferenceSystem`](../../../aspose.gis.spatialreferencing/spatialreferencesystem/)此几何的 ： 对于预计的 SRS 公差为 0.001 米（以 SRS 为单位）对于地理 SRS 公差是`1e-5`度（SRS 单位）对于未知的 SRS 公差是`1e-5` 有关应用哪些转换的更多详细信息，请参阅[`ToLinearGeometry`](../../igeometrycollection/tolineargeometry/)规范.
...
| 范围 | 类型 | 描述 |
| --- | --- | --- |
| tolerance | Double | 的`宽容`使用。结果保证小于`宽容`远离 the 弯曲的几何图形。 |
