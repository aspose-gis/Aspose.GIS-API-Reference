...
description: "IGeometry 方法。确定此几何体与指定几何体的 DE9IM 相交矩阵是否匹配提供的模式"
type: docs
weight: 290
...
| --- | --- | --- |
| 其他 | IGeometry | 一个几何体。 |
| intersectionPatternMatrix | 字符串 | 用于匹配的模式。该值应为长度为 9 的字符串。字符串的每个字符表示交叉的预期维度：字符 0 - 两个几何体内部之间。字符 1 - 此几何体的内部与另一个几何体的边界之间。字符 2 - 此几何体的内部与另一个几何体的外部之间。字符 3 - 此几何体的边界与另一个几何体的内部之间。字符 4 - 两个几何体的边界之间。字符 5 - 此几何体的边界与另一个几何体的外部之间。字符 6 - 此几何体的外部与另一个几何体的内部之间。字符 7 - 此几何体的外部与另一个几何体的边界之间。字符 8 - 两个几何体的外部之间。每个字符的可能取值为：* - 任意值；F - 无交叉；T - 任意交叉；0 - 点交叉（例如共享点）；1 - 线交叉（例如共享线段）；2 - 面交叉（例如共享多边形部分）；例如，交叉模式 "F0*******" 表示几何体内部之间不应有交叉，而几何体边界之间的交叉必须是一个点。有关交叉矩阵模式的更多细节，请参阅 OpenGIS Simple Features Specification。 |
