...
| [WriteBoundingBoxes](../../aspose.gis.formats.geojson/geojsonoptions/writeboundingboxes) { get; set; } | 确定是否应包含 GeoJSON 对象的几何坐标范围信息。 如果设置为`true`，每个几何图形（非空）在添加到图层时都会生成一个成员“bbox”。 默认值为`false`. |
