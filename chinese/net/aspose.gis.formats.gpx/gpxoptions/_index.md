...
description: "Aspose.Gis.Formats.Gpx.GpxOptions 类。针对 GPX 格式的驱动程序特定选项"
type: docs
weight: 1980
...
| [MAttribute](../../aspose.gis.formats.gpx/gpxoptions/mattribute/) { get; set; } | 确定哪个 GPX 属性将被导出为航点、路线点和轨迹点的 'M' 坐标。行为与 [`ZAttribute`](./zattribute/) 相同，默认值为 `null`。 |
...
| [ZAttribute](../../aspose.gis.formats.gpx/gpxoptions/zattribute/) { get; set; } | 确定哪个 GPX 属性将被导出为航点、路线点和轨迹点的 'Z' 坐标。如果为 `null` - 则不会有属性导出为 'Z' 坐标。默认值为 "ele"。可能的值是所有可以表示为 double 的 GPX XML 属性名称（例如 "speed"、"magvar"、"geoidheight" 等）。 |
