...
description: 样式会影响几何图形的呈现方式 这些样式在文档部分中编码并且每个样式都有一个唯一标识符 这些在 kml 规范中被称为共享样式
type: docs
weight: 400
...
样式会影响几何图形的呈现方式。 这些样式在文档部分中编码，并且每个样式都有一个唯一标识符。 这些在 kml 规范中被称为“共享样式”。
...
| [Label](../../aspose.gis.formats.kml.styles/kmlfeaturestyle/label) { get; set; } | 指定如何绘制特征标签。 使用`null`表示缺少样式。 默认值为`null`。 |
