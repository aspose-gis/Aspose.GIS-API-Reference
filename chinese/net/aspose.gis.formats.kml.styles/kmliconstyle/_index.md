...
description: "Aspose.Gis.Formats.Kml.Styles.KmlIconStyle 类。指定在地球浏览器列表和地理视图中，如何绘制带有 kmlPoint 几何的 kmlPlacemarks 和 kmlPhotoOverlay 的图标。"
type: docs
weight: 2130
...
指定在地球浏览器的列表和地理视图中，如何绘制带有 kml:Point 几何的 kml:Placemarks 和 kml:PhotoOverlay 的图标。
...
| [HotSpot](../../aspose.gis.formats.kml.styles/kmliconstyle/hotspot/) { get; set; } | 指定锚定在 Placemark 中指定的 Point 上的图标参考点位置。默认值为 `null` 表示 HotSpot 缺失。 |
| [Resource](../../aspose.gis.formats.kml.styles/kmliconstyle/resource/) { get; set; } | 指定资源位置。默认值为 `null` 表示 Icon 缺失。 |
