---
title: Class KmlListStyle
second_title: Aspose.GIS for .NET API 参考
description: Aspose.Gis.Formats.Kml.Styles.KmlListStyle 班级. 指定功能在列表视图中的显示方式
type: docs
weight: 550
url: /zh/net/aspose.gis.formats.kml.styles/kmlliststyle/
---
## KmlListStyle class

指定功能在列表视图中的显示方式。

```csharp
public class KmlListStyle
```

## 构造函数

| 姓名 | 描述 |
| --- | --- |
| [KmlListStyle](kmlliststyle/)() | 默认构造函数。 |

## 特性

| 姓名 | 描述 |
| --- | --- |
| [BackgroundColor](../../aspose.gis.formats.kml.styles/kmlliststyle/backgroundcolor/) { get; set; } | 指定图形元素的背景颜色。默认值为White. |
| [ItemIcons](../../aspose.gis.formats.kml.styles/kmlliststyle/itemicons/) { get; set; } | 列表视图中使用的图标，反映文件夹或链接提取的状态。 默认值为`null`表示没有图标。 |
| [ItemType](../../aspose.gis.formats.kml.styles/kmlliststyle/itemtype/) { get; set; } | 指定 kml:Folder 及其内容应如何显示为列表视图中的项目。 默认值为Check. |
| [MaxSnippetLines](../../aspose.gis.formats.kml.styles/kmlliststyle/maxsnippetlines/) { get; set; } | 指定要在列表视图中显示的最大行数。默认值为“2”。 |

### 也可以看看

* 命名空间 [Aspose.Gis.Formats.Kml.Styles](../../aspose.gis.formats.kml.styles/)
* 部件 [Aspose.GIS](../../)


