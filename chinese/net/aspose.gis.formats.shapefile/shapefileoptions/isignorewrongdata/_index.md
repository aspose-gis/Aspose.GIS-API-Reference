---
title: "ShapefileOptions.IsIgnoreWrongData"
second_title: "Aspose.GIS for .NET API 参考"
description: "ShapefileOptions 属性。确定在解析错误数据时是抛出异常还是为其设置默认值"
type: docs
weight: 20
url: /zh/net/aspose.gis.formats.shapefile/shapefileoptions/isignorewrongdata/
---
## ShapefileOptions.IsIgnoreWrongData property

确定在解析错误数据时是抛出异常还是为其设置默认值

```csharp
public bool IsIgnoreWrongData { get; set; }
```

### 另见

* class [ShapefileOptions](../)
* namespace [Aspose.Gis.Formats.Shapefile](../../shapefileoptions/)
* assembly [Aspose.GIS](../../../)


