...
| column_wkt | string | r/w | 获取或设置用于表示几何的 Well-Known Text 所在列的名称。<br/>            默认值为 <see langword=\"null\" />。 |
...
| delimiter | 字符 | r/w | 获取或设置用于分隔值的分隔符字符。<br/>            默认值为 ','. |
| double_quote_escape | 字符 | r/w | 获取或设置用于转义双引号的字符。<br/>            默认值为 '\\"'。 |
| has_attribute_names | bool | r/w | 确定是否存在带有属性名称的标题行。<br/>            默认值为 <see langword=\"true\" />。 |
...
| start_line_number | int | r/w | 获取或设置在读取数据时第一行的零基行号。<br/>            默认值为 0。 |
