...
| m_attribute | string | r/w | 确定哪个 GPX 属性将被导出为航点、路线点和轨迹点的 'M' 坐标。<br/>            行为与 [GpxOptions.z_attribute](/psd/python-net/aspose.gis.formats.gpx/gpxoptions/) 相同，默认值为 <see langword=\"null\" />。 |
...
| z_attribute | string | r/w | 确定哪个 GPX 属性将被导出为航点、路线点和轨迹点的 'Z' 坐标。<br/>            如果 <see langword=\"null\" /> - 则不会有属性导出为 'Z' 坐标。<br/>            默认值为 \"ele\"。<br/>            可取值为所有可以表示为 double 的 GPX XML 属性名称（例如 \"speed\", \"magvar\", \"geoidheight\" 等）。 |
