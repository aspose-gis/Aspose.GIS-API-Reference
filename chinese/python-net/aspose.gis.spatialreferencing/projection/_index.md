...
| 名称 | string | r | 此对象的名称。 |
| parameters_names | System.Collections.Generic.IList<string> | r | 获取分配给此投影的参数名称的可枚举集合 |
## **Methods**
| **Name** | **Description** |
| :- | :- |
| [get_parameter_value(name, type)](#get_parameter_value_name_type_1) | 获取此投影中具有指定名称的参数。 |
| [is_equivalent(other)](#is_equivalent_other_2) | 确定两个投影是否等价。等价投影以相同方式将（经度，纬度）映射到（x，y）<br/>            。 |
| [try_get_parameter_value(name, type)](#try_get_parameter_value_name_type_3) | 获取此投影中具有指定名称的参数。如果不存在此参数，则返回 <see langword="null" />。 |
