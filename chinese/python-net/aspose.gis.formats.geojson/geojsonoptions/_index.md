...
| 描述 | string | r/w | 要素集合级别的描述（用于图层创建） |
...
| 名称 | string | r/w | 要素集合级别的名称（用于图层创建） |
...
| write_bounding_boxes | bool | r/w | 确定是否应在 GeoJSON 对象中包含其几何体的坐标范围信息。<br/>            如果设置为 <see langword="true" />，则在将几何体添加到图层时，为每个非空几何体生成成员 "bbox"。<br/>            默认值为 <see langword="false" />。 |
