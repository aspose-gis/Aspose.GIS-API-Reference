...
type: docs
weight: 20
url: /zh/python-net/aspose.gis.formats.gml/gmloptions/
---
...
| **Name** | **Description** |
| :- | :- |
| [GmlOptions()](#GmlOptions__1) | 创建新实例。 |
## **Properties**
| **Name** | **Type** | **Access** | **Description** |
| :- | :- | :- | :- |
| application_namespace | string | r/w | 此指定用于生成 gml 文档的自定义命名空间，作为应用程序命名空间使用。 |
...
| load_schemas_from_internet | bool | r/w | 确定是否允许 Aspose.GIS 从 Internet 加载 XML 架构。<br/>            如果设置为 <see langword="false" />，具有不以 'file://' 开头的绝对 URI 的架构将不会被加载。<br/>            默认值为 <see langword="false" />。 |
...
| restore_schema | bool | r/w | 确定是否允许 Aspose.GIS 解析在缺少或无法加载 XML 架构的 Gml 文件中的属性。<br/>            如果设置为 <see langword="true" />，Aspose.GIS 读取器不需要 XML 架构的存在。<br/>            默认值为 <see langword="false" />。 |
| schema_location | string | r/w | 以空格分隔的 URI 对列表。每对中的第一个 URI 是命名空间的 URI，第二个 URI 是该命名空间的 XML 架构路径。<br/>            如果设置为 <see langword="null" />，[GmlDriver](/psd/python-net/aspose.gis.formats.gml/gmldriver/) 将尝试从文档根元素读取 schemaLocation。<br/>            默认值为 <see langword="null" />。 |
