---
title: "GmlOptions 클래스"
type: docs
weight: 20
url: /ko/python-net/aspose.gis.formats.gml/gmloptions/
---
...
| **Name** | **Description** |
| :- | :- |
| [GmlOptions()](#GmlOptions__1) | 새 인스턴스를 생성합니다. |
## **Properties**
| **Name** | **Type** | **Access** | **Description** |
| :- | :- | :- | :- |
| application_namespace | string | r/w | 이것은 gml 문서를 생성하기 위해 애플리케이션 네임스페이스로 사용되는 사용자 지정 네임스페이스를 지정합니다. |
...
| load_schemas_from_internet | bool | r/w | Aspose.GIS가 인터넷에서 XML 스키마를 로드하도록 허용되는지 여부를 결정합니다.<br/>            <see langword="false" /> 로 설정하면 'file://' 로 시작하지 않는 절대 URI를 가진 스키마가 로드되지 않습니다.<br/>            기본값은 <see langword="false" /> 입니다. |
...
| restore_schema | bool | r/w | Aspose.GIS가 XML 스키마가 없거나 로드할 수 없는 Gml 파일의 속성을 구문 분석하도록 허용되는지 여부를 결정합니다.<br/>            <see langword="true" /> 로 설정하면 Aspose.GIS 리더는 XML 스키마의 존재를 요구하지 않습니다.<br/>            기본값은 <see langword="false" /> 입니다. |
| schema_location | string | r/w | 공백으로 구분된 URI 쌍 목록입니다. 각 쌍의 첫 번째 URI는 네임스페이스의 URI이며, 두 번째 URI는 해당 네임스페이스의 XML 스키마 경로입니다.<br/>            <see langword="null" /> 로 설정하면 [GmlDriver](/psd/python-net/aspose.gis.formats.gml/gmldriver/)가 문서 루트 요소에서 schemaLocation을 읽으려고 시도합니다.<br/>            기본값은 <see langword="null" /> 입니다. |
