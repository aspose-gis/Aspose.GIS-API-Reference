---
title: "CurvePolygon 클래스"
type: docs
weight: 40
url: /ko/python-net/aspose.gis.geometries/curvepolygon/
---
...
| **Name** | **Description** |
| :- | :- |
| [CurvePolygon()](#CurvePolygon__1) | 새로운 [CurvePolygon](/psd/python-net/aspose.gis.geometries/curvepolygon/) 클래스 인스턴스를 초기화합니다. |
| [CurvePolygon(exterior_ring)](#CurvePolygon_exterior_ring_2) | 새로운 [CurvePolygon](/psd/python-net/aspose.gis.geometries/curvepolygon/) 클래스 인스턴스를 초기화합니다. |
| [CurvePolygon(exterior_ring, interior_rings)](#CurvePolygon_exterior_ring_interior_rings_3) | 새로운 [CurvePolygon](/psd/python-net/aspose.gis.geometries/curvepolygon/) 클래스 인스턴스를 초기화합니다. |
## **Properties**
...
| exterior_ring | [ICurve](/psd/python-net/aspose.gis.geometries/icurve) | r/w | 외부 링을 가져옵니다. |
...
| interior_rings_count | int | r | 내부 링의 개수를 가져옵니다. |
...
| spatial_reference_system | [SpatialReferenceSystem](/psd/python-net/aspose.gis.spatialreferencing/spatialreferencesystem/) | r/w | 이 인스턴스의 SpatialReferenceSystem을 가져옵니다.<br/>            이 속성은 <see langword=\"null\" />일 수 있으며, SpatialReferenceSystem이 알려지지 않았습니다.<br/>            새로운 SpatialReferenceSystem을 할당해도 좌표 변환이 수행되지 않고, 참조만 변경됩니다. |
## **Methods**
| **Name** | **Description** |
| :- | :- |
| [add_interior_ring(ring)](#add_interior_ring_ring_1) | 내부 링을 추가합니다. |
| [as_binary()](#as_binary__2) | 이 지오메트리를 Well-Known Binary 표현으로 변환합니다. |
| [as_binary(variant)](#as_binary_variant_3) | 이 지오메트리를 Well-Known Binary 표현으로 변환합니다. |
...
| [as_image(output_path, width, height, renderer, symbolizer)](#as_image_output_path_width_height_renderer_symbolizer_5) | 이 지오메트리를 이미지 표현으로 내보냅니다. |
| [as_image(width, height, renderer, symbolizer)](#as_image_width_height_renderer_symbolizer_6) | 이 지오메트리를 이미지 표현으로 내보냅니다. |
| [as_text()](#as_text__7) | 이 지오메트리를 Well-Known Text 표현으로 변환합니다. |
| [as_text(variant)](#as_text_variant_8) | 이 지오메트리를 Well-Known Text 표현으로 변환합니다. |
| [as_text(variant, format)](#as_text_variant_format_9) | 이 지오메트리를 Well-Known Text 표현으로 변환합니다. |
| [clone()](#clone__10) | 이 인스턴스를 복제합니다. |
| [covered_by(other)](#covered_by_other_11) | 지정된 지오메트리에 의해 이 지오메트리가 포함되는지 여부를 판단합니다. |
| [covers(other)](#covers_other_12) | 이 지오메트리가 지정된 지오메트리를 포함하는지 여부를 판단합니다. |
| [crosses(other)](#crosses_other_13) | 이 기하와 지정된 기하가 교차하는지 확인합니다. |
| [difference(other)](#difference_other_14) | 이 기하에서 지정된 기하를 빼냅니다. |
| [disjoint(other)](#disjoint_other_15) | 이 기하가 지정된 기하와 분리되어 있는지 확인합니다. |
| [from_binary(wkb)](#from_binary_wkb_16) | Well-Known Binary 표현에서 기하를 생성합니다. |
| [from_binary(wkb, spatial_reference_system)](#from_binary_wkb_spatial_reference_system_17) | Well-Known Binary 표현에서 기하를 생성합니다. |
| [from_text(wkt)](#from_text_wkt_18) | Well-Known Text 표현에서 기하를 생성합니다. |
| [from_text(wkt, spatial_reference_system)](#from_text_wkt_spatial_reference_system_19) | Well-Known Text 표현에서 기하를 생성합니다. |
| [get_area()](#get_area__20) | 이 기하의 면적을 계산합니다. |
| [get_buffer(distance, quadrant_segments)](#get_buffer_distance_quadrant_segments_21) | 이 기하 주변에 버퍼 영역을 계산합니다. |
| [get_centroid()](#get_centroid__22) | 이 기하의 중심점을 계산합니다. |
| [get_convex_hull()](#get_convex_hull__23) | 이 기하의 볼록 껍질을 계산합니다. |
| [get_distance_to(other)](#get_distance_to_other_24) | 이 기하와 지정된 기하 사이의 최소 거리를 계산합니다. |
| [get_extent()](#get_extent__25) | 이 기하의 경계 범위를 계산하고 반환합니다. |
| [get_interior_ring(index)](#get_interior_ring_index_26) | 인덱스로 내부 링을 가져옵니다. |
| [get_length()](#get_length__27) | 이 기하의 길이를 계산합니다. |
| [get_point_on_surface()](#get_point_on_surface__28) | 이 곡선 다각형 위에 반드시 존재하는 점을 찾습니다. |
| [intersection(other)](#intersection_other_29) | 이 기하와 지정된 기하 사이의 교차점을 구축합니다. |
| [intersects(extent)](#intersects_extent_30) | 이 기하가 지정된 범위와 교차하는지 확인합니다. |
| [intersects(other)](#intersects_other_31) | 이 기하와 지정된 기하가 교차하는지 확인합니다. |
| [overlaps(other)](#overlaps_other_32) | 이 기하가 지정된 기하와 겹치는지 확인합니다. |
| [relate(other, intersection_pattern_matrix)](#relate_other_intersection_pattern_matrix_33) | 이 기하와 지정된 기하의 DE-9IM 교차 매트릭스가 제공된 패턴과 일치하는지 확인합니다. |
| [replace_polygons_by_lines()](#replace_polygons_by_lines__34) | 이 기하를 선으로 표현한 다각형을 가져옵니다. |
| [round_m(digits)](#round_m_digits_35) | M 좌표를 지정된 소수 자릿수로 반올림합니다. |
| [round_xy(digits)](#round_xy_digits_36) | X 및 Y 좌표를 지정된 소수 자릿수로 반올림합니다. |
| [round_z(digits)](#round_z_digits_37) | Z 좌표를 지정된 소수 자릿수로 반올림합니다. |
| set_empty() | 이 [Geometry](/psd/python-net/aspose.gis.geometries/geometry/)를 비웁니다. |
| [spatially_contains(other)](#spatially_contains_other_38) | 이 기하가 지정된 기하를 공간적으로 포함하는지 확인합니다. |
| [spatially_equals(other)](#spatially_equals_other_39) | 이 기하가 지정된 기하와 공간적으로 같은지 확인합니다. |
| [sym_difference(other)](#sym_difference_other_40) | 이 기하와 지정된 기하 사이의 대칭 차집합을 만듭니다. |
| [to_editable()](#to_editable__41) | 이 기하의 편집 가능한 복사본을 가져옵니다. |
| [to_linear_geometry()](#to_linear_geometry__42) | 기본 <c>tolerance</c>를 사용하여 이 기하의 근사 또는 동등한 비곡선 버전을 가져옵니다. |
| [to_linear_geometry(tolerance)](#to_linear_geometry_tolerance_43) | 지정된 <c>tolerance</c>를 사용하여 이 기하의 근사 또는 동등한 비곡선 버전을 가져옵니다. |
| [to_svg(extent)](#to_svg_extent_44) | 이 기하를 Svg 표현으로 변환합니다. |
| [touches(other)](#touches_other_45) | 이 기하와 지정된 기하가 접하는지 판단합니다. |
| [union(other)](#union_other_46) | 이 기하와 지정된 기하를 합칩니다. |
| [union(other)](#union_other_47) | 이 기하와 지정된 기하를 합칩니다. |
| [within(extent)](#within_extent_48) | 이 기하가 지정된 범위 내에 있는지 판단합니다. |
| [within(other)](#within_other_49) | 이 기하가 지정된 기하 내에 있는지 판단합니다. |
...
새로운 [CurvePolygon](/psd/python-net/aspose.gis.geometries/curvepolygon/) 클래스 인스턴스를 초기화합니다.
...
| 매개변수 | 형식 | 설명 |
| :- | :- | :- |
| exterior_ring | [ICurve](/psd/python-net/aspose.gis.geometries/icurve) | 외부 링. |
...
| :- | :- | :- |
| exterior_ring | [ICurve](/psd/python-net/aspose.gis.geometries/icurve) | 외부 링. |
| interior_rings | System.Collections.Generic.IEnumerable<ICurve> | 내부 링들. |
...
| 매개변수 | 형식 | 설명 |
| :- | :- | :- |
| ring | [ICurve](/psd/python-net/aspose.gis.geometries/icurve) | 추가할 링. |
...
| 형식 | 설명 |
| :- | :- |
| [ICurve](/psd/python-net/aspose.gis.geometries/icurve) | 내부 링. |
...
이 곡선 다각형 위에 반드시 존재하는 점을 찾습니다.

**Returns**

| 형식 | 설명 |
| :- | :- |
| [IPoint](/psd/python-net/aspose.gis.geometries/ipoint) | 이 곡선 다각형 위의 점입니다. 내부가 없을 경우 빈 점을 반환합니다. |
...
| 형식 | 설명 |
| :- | :- |
| [CurvePolygon](/psd/python-net/aspose.gis.geometries/curvepolygon) | 이 기하의 편집 가능한 복사본. |
...
| 형식 | 설명 |
| :- | :- |
| [IPolygon](/psd/python-net/aspose.gis.geometries/ipolygon) | 이 <c>ISurface</c>에 근사하거나 동등한 [IPolygon](/psd/python-net/aspose.gis.geometries/ipolygon/).<br/>            이는 <DOM Element: class at 0x2a1793609d0>.ISurface.to_linear_geometry()(float)와 동일하며,<br/>            기본 <c>tolerance</c>을 사용합니다. 기본 <c>tolerance</c> 값은 이 기하의 [SpatialReferenceSystem](/psd/python-net/aspose.gis.spatialreferencing/spatialreferencesystem/)에 따라 달라집니다:<br/>            <ul><br/>            <li> 투영된 SRS의 경우 허용오차는 0.001 미터(SRS 단위) 입니다 </li><br/>            <li> 지리적 SRS의 경우 허용오차는 <c>1e-5</c>도(SRS 단위) 입니다 </li><br/>            <li> 알 수 없는 SRS의 경우 허용오차는 <c>1e-5</c> 입니다 </li><br/>            </ul><br/>            적용되는 변환에 대한 자세한 내용은 <DOM Element: class at 0x2a1793609d0>.ISurface.to_linear_geometry()(float) 사양을 참조하십시오. |
...
| 형식 | 설명 |
| :- | :- |
| [IPolygon](/psd/python-net/aspose.gis.geometries/ipolygon) | 이 <c>ISurface</c>에 근사하거나 동등한 [IPolygon](/psd/python-net/aspose.gis.geometries/ipolygon/):<br/>             <ul><br/>             이 객체가 자체적으로 [IPolygon](/psd/python-net/aspose.gis.geometries/ipolygon/)인 경우 결과는 이 객체와 동일합니다<br/>             이 객체가 [IPolygon](/psd/python-net/aspose.gis.geometries/ipolygon/)이 아닌 경우 선형화되어 [IPolygon](/psd/python-net/aspose.gis.geometries/ipolygon/)이 생성됩니다<br/>            </ul> |
