---
title: "GeoJsonOptions 클래스"
type: docs
weight: 20
url: /ko/python-net/aspose.gis.formats.geojson/geojsonoptions/
---
...
| **Name** | **Description** |
| :- | :- |
| [GeoJsonOptions()](#GeoJsonOptions__1) | 새 인스턴스를 생성합니다. |
## **Properties**
...
| 설명 | string | r/w | 레이어 생성을 위한 피처 컬렉션 수준의 설명 |
...
| 이름 | string | r/w | 레이어 생성을 위한 피처 컬렉션 수준의 이름 |
| nested_properties_separator | string | r/w | 중첩 속성의 구성 요소를 구분하는 데 사용되는 문자열을 가져오거나 설정합니다.<br/>            기본값은 "_"입니다. |
| read_bounding_boxes | bool | r/w | Bounding Boxes('bbox')를 'bbox_0', 'bbox_1' 등 이름의 속성으로 읽어야 하는지 여부를 결정합니다.<br/>            기본값은 <see langword="false" />입니다.<br/>            [GeoJsonOptions.nested_properties_separator](/psd/python-net/aspose.gis.formats.geojson/geojsonoptions/) 문자열은 bbox_0, bbox_1,.. 이름에 사용됩니다. |
...
| write_bounding_boxes | bool | r/w | GeoJSON 객체에 해당 기하 도형의 좌표 범위 정보가 포함되어야 하는지 여부를 결정합니다.<br/>            <see langword="true" /> 로 설정하면, 레이어에 추가될 때 각 기하 도형(null이 아닌)에 대해 "bbox" 멤버가 생성됩니다.<br/>            기본값은 <see langword="false" />입니다. |
