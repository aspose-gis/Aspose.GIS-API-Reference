---
title: "Класс GpxOptions"
type: docs
weight: 20
url: /ru/python-net/aspose.gis.formats.gpx/gpxoptions/
---
...
| **Name** | **Описание** |
| :- | :- |
| [GpxOptions()](#GpxOptions__1) | Создать новый экземпляр. |
## **Properties**
...
| m_attribute | string | r/w | Определяет, какой атрибут GPX будет экспортирован как координата 'M' точек маршрута, пунктов маршрута и точек трека.<br/>            Поведение такое же, как у [GpxOptions.z_attribute](/psd/python-net/aspose.gis.formats.gpx/gpxoptions/), по умолчанию <see langword=\"null\" />. |
...
| nested_attribute_separator | string | r | Строка для разделения имени вложенного атрибута и его индексов. По умолчанию двойное подчеркивание \"__\". |
| read_nested_attributes | bool | r/w | Определяет, содержат ли точки GPX, такие как 'trkpt' и 'rtept', вложенные атрибуты и должны ли они быть прочитаны. По умолчанию <see langword=\"false\" />. |
...
| z_attribute | string | r/w | Определяет, какой атрибут GPX будет экспортирован как координата 'Z' точек маршрута, пунктов маршрута и точек трека.<br/>            Если <see langword=\"null\" /> — никакой атрибут не будет экспортирован как координата 'Z'.<br/>            По умолчанию \"ele\".<br/>            Возможные значения — имена всех атрибутов GPX XML, которые могут быть представлены как double (например, \"speed\", \"magvar\", \"geoidheight\" и т.д.) |
