---
title: "Класс CurvePolygon"
type: docs
weight: 40
url: /ru/python-net/aspose.gis.geometries/curvepolygon/
---
...
| **Name** | **Описание** |
| :- | :- |
| [CurvePolygon()](#CurvePolygon__1) | Инициализирует новый экземпляр класса [CurvePolygon](/psd/python-net/aspose.gis.geometries/curvepolygon/). |
| [CurvePolygon(exterior_ring)](#CurvePolygon_exterior_ring_2) | Инициализирует новый экземпляр класса [CurvePolygon](/psd/python-net/aspose.gis.geometries/curvepolygon/). |
| [CurvePolygon(exterior_ring, interior_rings)](#CurvePolygon_exterior_ring_interior_rings_3) | Инициализирует новый экземпляр класса [CurvePolygon](/psd/python-net/aspose.gis.geometries/curvepolygon/). |
## **Properties**
...
| exterior_ring | [ICurve](/psd/python-net/aspose.gis.geometries/icurve) | r/w | Получает внешнее кольцо. |
...
| interior_rings_count | int | r | Получает количество внутренних колец. |
...
| **Name** | **Описание** |
| :- | :- |
| [add_interior_ring(ring)](#add_interior_ring_ring_1) | Добавляет внутреннее кольцо. |
...
| [get_interior_ring(index)](#get_interior_ring_index_26) | Получает внутреннее кольцо по его индексу. |
| [get_length()](#get_length__27) | Вычисляет длину этой геометрии. |
| [get_point_on_surface()](#get_point_on_surface__28) | Находит точку, которая гарантированно находится на этом изогнутом полигоне. |
...
| [replace_polygons_by_lines()](#replace_polygons_by_lines__34) | Получает полигоны, представленные в виде линий этой геометрии. |
| [round_m(digits)](#round_m_digits_35) | Округляет координату M до указанного количества знаков после запятой. |
| [round_xy(digits)](#round_xy_digits_36) | Округляет координаты X и Y до указанного количества знаков после запятой. |
| [round_z(digits)](#round_z_digits_37) | Округляет координату Z до указанного количества знаков после запятой. |
| set_empty() | Делает эту [Geometry](/psd/python-net/aspose.gis.geometries/geometry/) пустой. |
| [spatially_contains(other)](#spatially_contains_other_38) | Определяет, содержит ли эта геометрия пространственно указанную геометрию. |
| [spatially_equals(other)](#spatially_equals_other_39) | Определяет, равна ли эта геометрия пространственно указанной геометрии. |
| [sym_difference(other)](#sym_difference_other_40) | Создает симметричную разность между этой геометрией и указанной геометрией. |
| [to_editable()](#to_editable__41) | Получает редактируемую копию этой геометрии. |
| [to_linear_geometry()](#to_linear_geometry__42) | Получает приближённую или эквивалентную нелинейную версию этой геометрии, используя значение <c>tolerance</c> по умолчанию. |
| [to_linear_geometry(tolerance)](#to_linear_geometry_tolerance_43) | Получает приближённую или эквивалентную нелинейную версию этой геометрии, используя указанное значение <c>tolerance</c>. |
| [to_svg(extent)](#to_svg_extent_44) | Преобразует эту геометрию в представление Svg. |
| [touches(other)](#touches_other_45) | Определяет, соприкасаются ли эта геометрия и указанная геометрия. |
| [union(other)](#union_other_46) | Объединяет эту геометрию и указанную геометрию. |
| [union(other)](#union_other_47) | Объединяет эту геометрию и указанную геометрию. |
| [within(extent)](#within_extent_48) | Определяет, находится ли эта геометрия внутри указанного охвата. |
| [within(other)](#within_other_49) | Определяет, находится ли эта геометрия внутри указанной геометрии. |
...
Инициализирует новый экземпляр класса [CurvePolygon](/psd/python-net/aspose.gis.geometries/curvepolygon/).
...
| :- | :- | :- |
| exterior_ring | [ICurve](/psd/python-net/aspose.gis.geometries/icurve) | Внешнее кольцо. |
...
| :- | :- | :- |
| exterior_ring | [ICurve](/psd/python-net/aspose.gis.geometries/icurve) | Внешнее кольцо. |
| interior_rings | System.Collections.Generic.IEnumerable<ICurve> | Внутренние кольца. |
...
| Параметр | Тип | Описание |
| :- | :- | :- |
| ring | [ICurve](/psd/python-net/aspose.gis.geometries/icurve) | Кольцо для добавления. |
...
| Тип | Описание |
| :- | :- |
| [ICurve](/psd/python-net/aspose.gis.geometries/icurve) | Внутреннее кольцо. |
...
Находит точку, которая гарантированно находится на этом изогнутом полигоне.

**Returns**

| Тип | Описание |
| :- | :- |
| [IPoint](/psd/python-net/aspose.gis.geometries/ipoint) | Точка на этом изогнутом полигоне. Пустая точка, если у этого изогнутого полигона нет внутренности. |
...
| Тип | Описание |
| :- | :- |
| [CurvePolygon](/psd/python-net/aspose.gis.geometries/curvepolygon) | Редактируемая копия этой геометрии. |
...
| Тип | Описание |
| :- | :- |
| [IPolygon](/psd/python-net/aspose.gis.geometries/ipolygon) | Полигон [IPolygon](/psd/python-net/aspose.gis.geometries/ipolygon/), который приближен или эквивалентен этому <c>ISurface</c>.<br/>            Это эквивалент вызова <DOM Element: class at 0x2a1793609d0>.ISurface.to_linear_geometry()(float) с<br/>            значением <c>tolerance</c> по умолчанию. Значение <c>tolerance</c> по умолчанию зависит от [SpatialReferenceSystem](/psd/python-net/aspose.gis.spatialreferencing/spatialreferencesystem/)<br/>            этой геометрии:<br/>            <ul><br/>            <li> Для проецируемой СК допускается 0.001 метра (в единицах СК) </li><br/>            <li> Для географической СК допускается <c>1e-5</c> градусов (в единицах СК) </li><br/>            <li> Для неизвестной СК допускается <c>1e-5</c> </li><br/>            </ul><br/>            Для получения более подробной информации о применяемых преобразованиях см. спецификацию <DOM Element: class at 0x2a1793609d0>.ISurface.to_linear_geometry()(float). |
...
| Тип | Описание |
| :- | :- |
| [IPolygon](/psd/python-net/aspose.gis.geometries/ipolygon) | Полигон [IPolygon](/psd/python-net/aspose.gis.geometries/ipolygon/), который приближен или эквивалентен этому <c>ISurface</c>:<br/>             <ul><br/>             Если этот объект является [IPolygon](/psd/python-net/aspose.gis.geometries/ipolygon/) сам по себе, результат эквивалентен этому объекту<br/>             Если этот объект не является [IPolygon](/psd/python-net/aspose.gis.geometries/ipolygon/), он линеаризуется и создаётся [IPolygon](/psd/python-net/aspose.gis.geometries/ipolygon/)<br/>            </ul> |
