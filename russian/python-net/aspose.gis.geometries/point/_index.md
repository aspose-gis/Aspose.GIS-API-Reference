...
type: docs
weight: 300
url: /ru/python-net/aspose.gis.geometries/point/
---
...
| **Name** | **Описание** |
| :- | :- |
| [Point()](#Point__1) | Инициализирует новый экземпляр класса [Point](/psd/python-net/aspose.gis.geometries/point/). |
| [Point(other)](#Point_other_2) | Инициализирует новый экземпляр класса [Point](/psd/python-net/aspose.gis.geometries/point/). |
| [Point(x, y)](#Point_x_y_3) | Инициализирует новый экземпляр класса [Point](/psd/python-net/aspose.gis.geometries/point/). |
| [Point(x, y, z)](#Point_x_y_z_4) | Инициализирует новый экземпляр класса [Point](/psd/python-net/aspose.gis.geometries/point/). |
| [Point(x, y, z, m)](#Point_x_y_z_m_5) | Инициализирует новый экземпляр класса [Point](/psd/python-net/aspose.gis.geometries/point/). |
## **Properties**
...
| has_m | bool | r/w | Получает значение, указывающее, имеет ли этот экземпляр координату M. |
...
| m | double | r/w | Получает или задает значение для координаты m. |
...
| x | double | r/w | Получает или задает значение для координаты x. |
| y | double | r/w | Получает или задает значение для координаты y. |
| z | double | r/w | Получает или задает значение для координаты z. |
## **Methods**
| **Name** | **Описание** |
| :- | :- |
...
| [from_binary(wkb)](#from_binary_wkb_15) | Создаёт геометрию из её представления в формате Well-Known Binary. |
| [from_binary(wkb, spatial_reference_system)](#from_binary_wkb_spatial_reference_system_16) | Создаёт геометрию из её представления в формате Well-Known Binary. |
| [from_text(wkt)](#from_text_wkt_17) | Создаёт геометрию из её представления в формате Well-Known Text. |
| [from_text(wkt, spatial_reference_system)](#from_text_wkt_spatial_reference_system_18) | Создаёт геометрию из её представления в формате Well-Known Text. |
| [get_area()](#get_area__19) | Вычисляет площадь этой геометрии. |
| [get_buffer(distance, quadrant_segments)](#get_buffer_distance_quadrant_segments_20) | Вычисляет буферную область вокруг этой геометрии. |
| [get_centroid()](#get_centroid__21) | Вычисляет центр масс этой геометрии. |
| [get_convex_hull()](#get_convex_hull__22) | Вычисляет выпуклую оболочку этой геометрии. |
| [get_distance_to(other)](#get_distance_to_other_23) | Вычисляет минимальное расстояние между этой геометрией и указанной геометрией. |
| [get_extent()](#get_extent__24) | Вычисляет и возвращает ограничивающий объём этой геометрии. |
| [get_length()](#get_length__25) | Вычисляет длину этой геометрии. |
| [intersection(other)](#intersection_other_26) | Создаёт пересечение между этой геометрией и указанной геометрией. |
| [intersects(extent)](#intersects_extent_27) | Определяет, пересекает ли эта геометрия указанный охват. |
| [intersects(other)](#intersects_other_28) | Определяет, пересекаются ли эта геометрия и указанная геометрия. |
| [overlaps(other)](#overlaps_other_29) | Определяет, перекрывается ли эта геометрия с указанной геометрией. |
| [relate(other, intersection_pattern_matrix)](#relate_other_intersection_pattern_matrix_30) | Определяет, соответствует ли матрица пересечения DE-9IM этой геометрии и указанной геометрии заданному шаблону. |
| [replace_polygons_by_lines()](#replace_polygons_by_lines__31) | Получает полигоны, представленные в виде линий этой геометрии. |
| [round_m(digits)](#round_m_digits_32) | Округляет координату M до указанного количества знаков после запятой. |
| [round_xy(digits)](#round_xy_digits_33) | Округляет координаты X и Y до указанного количества знаков после запятой. |
| [round_z(digits)](#round_z_digits_34) | Округляет координату Z до указанного количества знаков после запятой. |
| set_empty() | Делает эту [Geometry](/psd/python-net/aspose.gis.geometries/geometry/) пустой. |
| [spatially_contains(other)](#spatially_contains_other_35) | Определяет, содержит ли эта геометрия пространственно указанную геометрию. |
| [spatially_equals(other)](#spatially_equals_other_36) | Определяет, равна ли эта геометрия пространственно указанной геометрии. |
| [sym_difference(other)](#sym_difference_other_37) | Создает симметричную разность между этой геометрией и указанной геометрией. |
| [to_editable()](#to_editable__38) | Получает редактируемую копию этой геометрии. |
| [to_linear_geometry()](#to_linear_geometry__39) | Получает приближённую или эквивалентную нелинейную версию этой геометрии, используя значение <c>tolerance</c> по умолчанию. |
| [to_linear_geometry(tolerance)](#to_linear_geometry_tolerance_40) | Получает приближённую или эквивалентную нелинейную версию этой геометрии, используя указанное значение <c>tolerance</c>. |
| [to_svg(extent)](#to_svg_extent_41) | Преобразует эту геометрию в представление Svg. |
| [touches(other)](#touches_other_42) | Определяет, соприкасаются ли эта геометрия и указанная геометрия. |
| [union(other)](#union_other_43) | Объединяет эту геометрию и указанную геометрию. |
| [union(other)](#union_other_44) | Объединяет эту геометрию и указанную геометрию. |
| [within(extent)](#within_extent_45) | Определяет, находится ли эта геометрия внутри указанного охвата. |
| [within(other)](#within_other_46) | Определяет, находится ли эта геометрия внутри указанной геометрии. |
...
Инициализирует новый экземпляр класса [Point](/psd/python-net/aspose.gis.geometries/point/).
...
| :- | :- | :- |
| other | [IPoint](/psd/python-net/aspose.gis.geometries/ipoint) | Другой [IPoint](/psd/python-net/aspose.gis.geometries/ipoint/), из которого копировать данные. |
...
| Параметр | Тип | Описание |
| :- | :- | :- |
| расстояние | double | Ширина буферной зоны. |
...
| Тип | Описание |
| :- | :- |
| double | Если обе геометрии не являются [Geometry.is_empty](/psd/python-net/aspose.gis.geometries/geometry/), возвращается расстояние между ближайшими точками геометрий.<br/>            Если хотя бы одна геометрия пуста, возвращается -1. |
...
| Тип | Описание |
| :- | :- |
| [Point](/psd/python-net/aspose.gis.geometries/point) | Редактируемая копия этой геометрии. |
