---
title: "Класс LinearRing"
type: docs
weight: 240
url: /ru/python-net/aspose.gis.geometries/linearring/
---
...
| **Name** | **Описание** |
| :- | :- |
| [LinearRing()](#LinearRing__1) | Инициализирует новый экземпляр класса [LinearRing](/psd/python-net/aspose.gis.geometries/linearring/). |
| [LinearRing(collection)](#LinearRing_collection_2) | Инициализирует новый экземпляр класса [LinearRing](/psd/python-net/aspose.gis.geometries/linearring/). |
| [LinearRing(other)](#LinearRing_other_3) | Инициализирует новый экземпляр класса [LinearRing](/psd/python-net/aspose.gis.geometries/linearring/). |
## **Properties**
...
| **Name** | **Описание** |
| :- | :- |
| [add_point(point)](#add_point_point_1) | Добавляет точку в конец линии. |
| [add_point(x, y)](#add_point_x_y_2) | Добавляет точку в конец линии. |
| [add_point(x, y, z)](#add_point_x_y_z_3) | Добавляет точку в конец линии. |
| [add_point(x, y, z, m)](#add_point_x_y_z_m_4) | Добавляет точку в конец линии. |
...
| [is_clockwise()](#is_clockwise__33) | Определяет, имеет ли кольцо обход по часовой стрелке. |
| [overlaps(other)](#overlaps_other_34) | Определяет, перекрывается ли эта геометрия с указанной геометрией. |
| [relate(other, intersection_pattern_matrix)](#relate_other_intersection_pattern_matrix_35) | Определяет, соответствует ли матрица пересечения DE-9IM этой геометрии и указанной геометрии заданному шаблону. |
| [replace_polygons_by_lines()](#replace_polygons_by_lines__36) | Получает полигоны, представленные в виде линий этой геометрии. |
| reverse() | Изменяет порядок точек в этом [LineString](/psd/python-net/aspose.gis.geometries/linestring/) на обратный. |
| [round_m(digits)](#round_m_digits_37) | Округляет координату M до указанного количества знаков после запятой. |
| [round_xy(digits)](#round_xy_digits_38) | Округляет координаты X и Y до указанного количества знаков после запятой. |
| [round_z(digits)](#round_z_digits_39) | Округляет координату Z до указанного количества знаков после запятой. |
| set_empty() | Делает эту [Geometry](/psd/python-net/aspose.gis.geometries/geometry/) пустой. |
| [spatially_contains(other)](#spatially_contains_other_40) | Определяет, содержит ли эта геометрия пространственно указанную геометрию. |
| [spatially_equals(other)](#spatially_equals_other_41) | Определяет, равна ли эта геометрия пространственно указанной геометрии. |
| [sym_difference(other)](#sym_difference_other_42) | Создает симметричную разность между этой геометрией и указанной геометрией. |
| [to_editable()](#to_editable__43) | Получает редактируемую копию этой геометрии. |
| [to_linear_geometry()](#to_linear_geometry__44) | Получает приближённую или эквивалентную нелинейную версию этой геометрии, используя значение <c>tolerance</c> по умолчанию. |
| [to_linear_geometry(tolerance)](#to_linear_geometry_tolerance_45) | Получает приближённую или эквивалентную нелинейную версию этой геометрии, используя указанное значение <c>tolerance</c>. |
| [to_svg(extent)](#to_svg_extent_46) | Преобразует эту геометрию в представление Svg. |
| [touches(other)](#touches_other_47) | Определяет, соприкасаются ли эта геометрия и указанная геометрия. |
| [union(other)](#union_other_48) | Объединяет эту геометрию и указанную геометрию. |
| [union(other)](#union_other_49) | Объединяет эту геометрию и указанную геометрию. |
| [within(extent)](#within_extent_50) | Определяет, находится ли эта геометрия внутри указанного охвата. |
| [within(other)](#within_other_51) | Определяет, находится ли эта геометрия внутри указанной геометрии. |
...
Инициализирует новый экземпляр класса [LinearRing](/psd/python-net/aspose.gis.geometries/linearring/).
...
| Параметр | Тип | Описание |
| :- | :- | :- |
| other | [ILineString](/psd/python-net/aspose.gis.geometries/ilinestring) | Другая линия, из которой копировать данные. |
...
Определяет, имеет ли кольцо обход по часовой стрелке.

**Returns**

| Тип | Описание |
| :- | :- |
| bool | <see langword="true" /> если по часовой стрелке; иначе <see langword="false" />. |
...
| Тип | Описание |
| :- | :- |
| [LinearRing](/psd/python-net/aspose.gis.geometries/linearring) | Редактируемая копия этой геометрии. |
