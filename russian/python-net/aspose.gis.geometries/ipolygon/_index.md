---
title: "IPolygon Класс"
type: docs
weight: 210
url: /ru/python-net/aspose.gis.geometries/ipolygon/
---
...
| exterior_ring | [ILinearRing](/psd/python-net/aspose.gis.geometries/ilinearring) | r | Получает внешнее кольцо. |
...
| [get_interior_ring(index)](#get_interior_ring_index_21) | Получает внутреннее кольцо по его индексу. |
| [get_length()](#get_length__22) | Вычисляет длину этой геометрии. |
| [get_point_on_surface()](#get_point_on_surface__23) | Находит точку, гарантированно находящуюся на этой поверхности. |
| [intersection(other)](#intersection_other_24) | Создаёт пересечение между этой геометрией и указанной геометрией. |
| [intersects(extent)](#intersects_extent_25) | Определяет, пересекает ли эта геометрия указанный охват. |
| [intersects(other)](#intersects_other_26) | Определяет, пересекаются ли эта геометрия и указанная геометрия. |
| [overlaps(other)](#overlaps_other_27) | Определяет, перекрывается ли эта геометрия с указанной геометрией. |
| [relate(other, intersection_pattern_matrix)](#relate_other_intersection_pattern_matrix_28) | Определяет, соответствует ли матрица пересечения DE-9IM этой геометрии и указанной геометрии заданному шаблону. |
| [replace_polygons_by_lines()](#replace_polygons_by_lines__29) | Получает полигоны, представленные в виде линий этой геометрии. |
| [spatially_contains(other)](#spatially_contains_other_30) | Определяет, содержит ли эта геометрия пространственно указанную геометрию. |
| [spatially_equals(other)](#spatially_equals_other_31) | Определяет, равна ли эта геометрия пространственно указанной геометрии. |
| [sym_difference(other)](#sym_difference_other_32) | Создает симметричную разность между этой геометрией и указанной геометрией. |
| [to_editable()](#to_editable__33) | Получает редактируемую копию этой геометрии. |
| [to_linear_geometry()](#to_linear_geometry__34) | Получает приближённую или эквивалентную нелинейную версию этой геометрии, используя значение <c>tolerance</c> по умолчанию. |
| [to_linear_geometry(tolerance)](#to_linear_geometry_tolerance_35) | Получает приближённую или эквивалентную нелинейную версию этой геометрии, используя указанное значение <c>tolerance</c>. |
| [touches(other)](#touches_other_36) | Определяет, соприкасаются ли эта геометрия и указанная геометрия. |
| [union(other)](#union_other_37) | Объединяет эту геометрию и указанную геометрию. |
| [union(other)](#union_other_38) | Объединяет эту геометрию и указанную геометрию. |
| [within(extent)](#within_extent_39) | Определяет, находится ли эта геометрия внутри указанного охвата. |
| [within(other)](#within_other_40) | Определяет, находится ли эта геометрия внутри указанной геометрии. |
