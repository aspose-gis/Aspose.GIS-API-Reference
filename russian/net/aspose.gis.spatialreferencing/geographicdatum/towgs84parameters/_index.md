---
title: GeographicDatum.ToWgs84Parameters
second_title: Справочник по Aspose.GIS for .NET API
description: GeographicDatum свойство. Параметры BursaWolf которые можно использовать для преобразования координат в этой системе координат в координаты системы координат WGS84.
type: docs
weight: 80
url: /ru/net/aspose.gis.spatialreferencing/geographicdatum/towgs84parameters/
---
## GeographicDatum.ToWgs84Parameters property

Параметры BursaWolf, которые можно использовать для преобразования координат в этой системе координат в координаты системы координат WGS84.

```csharp
public BursaWolfParameters ToWgs84Parameters { get; }
```

### Смотрите также

* class [BursaWolfParameters](../../bursawolfparameters/)
* class [GeographicDatum](../)
* пространство имен [Aspose.Gis.SpatialReferencing](../../geographicdatum/)
* сборка [Aspose.GIS](../../../)


