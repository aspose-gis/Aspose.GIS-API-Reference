...
Этот метод автоматически преобразует значение в тип, запрошенный в параметре универсального типа.  Если слой не требует, чтобы его объекты имели значения для всех атрибутов, определенных для слоя, этот метод может дать сбой сInvalidOperationExceptionкогда запрашивается отсутствующее значение. При работе с такими слоями рассмотрите возможность использования[`GetValueOrDefault`](../getvalueordefault) .
...
Если слой не требует, чтобы его объекты имели значения для всех атрибутов, определенных для слоя, этот метод может дать сбой сInvalidOperationExceptionкогда запрашивается отсутствующее значение. При работе с такими слоями рассмотрите возможность использования[`GetValueOrDefault`](../getvalueordefault) .
