---
title: Class Drivers
second_title: Справочник по Aspose.GIS for .NET API
description: Aspose.Gis.Drivers сорт. Драйверы для всех поддерживаемых форматов.
type: docs
weight: 110
url: /ru/net/aspose.gis/drivers/
---
## Drivers class

Драйверы для всех поддерживаемых форматов.

```csharp
public static class Drivers
```

## Характеристики

| Имя | Описание |
| --- | --- |
| static [Csv](../../aspose.gis/drivers/csv/) { get; } | Драйвер для формата CSV. |
| static [EsriAscii](../../aspose.gis/drivers/esriascii/) { get; } | Драйвер для растрового формата Esri AscII. |
| static [EsriJson](../../aspose.gis/drivers/esrijson/) { get; } | Драйвер для формата EsriJson. |
| static [FileGdb](../../aspose.gis/drivers/filegdb/) { get; } | Драйвер для формата файловой базы геоданных ESRI. |
| static [GDAL](../../aspose.gis/drivers/gdal/) { get; } | Драйвер для формата GDAL. |
| static [GeoJson](../../aspose.gis/drivers/geojson/) { get; } | Драйвер для формата GeoJSON. |
| static [GeoTiff](../../aspose.gis/drivers/geotiff/) { get; } | Драйвер для растрового формата GeoTIFF или TIFF. |
| static [Gml](../../aspose.gis/drivers/gml/) { get; } | Драйвер для формата GML. |
| static [Gpx](../../aspose.gis/drivers/gpx/) { get; } | Драйвер для формата GPX. |
| static [InMemory](../../aspose.gis/drivers/inmemory/) { get; } | Драйвер для работы с данными в памяти. |
| static [Kml](../../aspose.gis/drivers/kml/) { get; } | Драйвер для формата KML. |
| static [MapInfoInterchange](../../aspose.gis/drivers/mapinfointerchange/) { get; } | Драйвер для формата обмена MapInfo. |
| static [MapInfoTab](../../aspose.gis/drivers/mapinfotab/) { get; } | Драйвер для формата вкладки MapInfo. |
| static [OsmXml](../../aspose.gis/drivers/osmxml/) { get; } | Драйвер для формата OSM XML. |
| static [PostGis](../../aspose.gis/drivers/postgis/) { get; } | Драйвер для базы данных PostGIS. |
| static [Shapefile](../../aspose.gis/drivers/shapefile/) { get; } | Драйвер для формата Shapefile. |
| static [SqlServer](../../aspose.gis/drivers/sqlserver/) { get; } | Драйвер для базы данных SQL Server. |
| static [TopoJson](../../aspose.gis/drivers/topojson/) { get; } | Драйвер для формата TopoJSON. |
| static [XyzTiles](../../aspose.gis/drivers/xyztiles/) { get; } | Драйвер для тайловой веб-карты, такой как OpenStreetMaps, Google Maps и т. д. |

### Смотрите также

* пространство имен [Aspose.Gis](../../aspose.gis/)
* сборка [Aspose.GIS](../../)


