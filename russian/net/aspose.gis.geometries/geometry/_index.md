...
| abstract [Dimension](../../aspose.gis.geometries/geometry/dimension) { get; } | Получает топологическую размерность этого[`Geometry`](../geometry) . Если размер неизвестен (например, для пустой коллекции GEOMETRYCOLLECTION)Point возвращается. |
