...
Геометрия без кривых. Это эквивалент[`ToLinearGeometry`](../tolineargeometry) с по умолчанию`толерантность` . По умолчанию`толерантность` определяется[`SpatialReferenceSystem`](../spatialreferencesystem) этой геометрии:  Для проектируемого SRS Допуск составляет 0,001 метра (в единицах SRS) Для географического SRS допуск равен`1е-5` градусы (в единицах SRS) Для неизвестного SRS Допуск равен`1е-5` Подробнее о применяемых преобразованиях см.[`ToLinearGeometry`](../tolineargeometry) спецификация.
