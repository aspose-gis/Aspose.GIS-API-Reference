...
description: IGeometry метод. Определяет перекрывается ли эта геометрия с указанной геометрией.
type: docs
weight: 280
url: /ru/net/aspose.gis.geometries/igeometry/overlaps/
---
...
Этот метод проверяет, перекрываются ли геометрии с точки зрения матрицы пересечения DE-9IM. Две геометрии перекрываются, если они имеют некоторые, но не все общие внутренние точки, и пересечение геометрий имеет ту же размерность, что и сами геометрии. На двоихPoint геометрия или двеSurface геометрии this метод эквивалентен: На двоихLine геометрии этот метод эквивалентен: Для двух геометрий с неравными[`Dimension`](../dimension/) этот метод всегда возвращает`false`. Дополнительные сведения о DE-9IM и отношении «пространственное перекрытие» см. в Спецификации простых функций OpenGIS.
