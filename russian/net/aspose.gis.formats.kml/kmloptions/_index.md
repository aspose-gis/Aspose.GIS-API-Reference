...
description: Aspose.Gis.Formats.Kml.KmlOptions сорт. Параметры драйвера для формата KML.
type: docs
weight: 410
url: /ru/net/aspose.gis.formats.kml/kmloptions/
---
## KmlOptions class

Параметры драйвера для формата KML.
...
| Имя | Описание |
| --- | --- |
| [KmlOptions](kmloptions/)() | Создать новый экземпляр. |

## Характеристики

| Имя | Описание |
| --- | --- |
| [AltitudeMode](../../aspose.gis.formats.kml/kmloptions/altitudemode/) { get; set; } | Позволяет указать AltitudeModes для использования с геометрией KML |
| [AutoId](../../aspose.gis.formats.kml/kmloptions/autoid/) { get; set; } | Автоматически генерировать идентификаторы |
...
| [DocumentId](../../aspose.gis.formats.kml/kmloptions/documentid/) { get; set; } | Используется для указания идентификатора корневого «Документа» node |
...
| [SymbolToReplaceInvalidChars](../../aspose.gis.formats.kml/kmloptions/symboltoreplaceinvalidchars/) { get; set; } | Определяет, какой символ будет использоваться для замены недопустимых символов при чтении. Замена пропускается, если значение равно '\0'. По умолчанию значение равно '\0' char. |
