...
| [LoadSchemasFromInternet](../../aspose.gis.formats.gml/gmloptions/loadschemasfrominternet) { get; set; } | Определяет, разрешено ли Aspose.GIS загружать схему XML из Интернета. Если установлено значение`false`, схемы с абсолютными URI, которые не начинаются с 'file://', не будут загружены. По умолчанию:`false`. |
...
| [XmlResolver](../../aspose.gis.formats.gml/gmloptions/xmlresolver) { get; set; } | A[`XmlResolver`](./xmlresolver)используется для разрешения внешних ресурсов. По умолчанию используетсяXmlUrlResolver. |
