...
Una geometría que no tiene geometrías de curvas. Este es el equivalente de[`ToLinearGeometry`](../tolineargeometry) con predeterminado`tolerancia` . Defecto`tolerancia` es definido por[`SpatialReferenceSystem`](../spatialreferencesystem) de esta geometría:  Para SRS proyectado, la tolerancia es de 0,001 metros (en unidades SRS) Para la tolerancia SRS geográfica es`1e-5` grados (en unidades SRS) Para SRS desconocido, la tolerancia es`1e-5` Para obtener más detalles sobre qué transformaciones se aplican, consulte[`ToLinearGeometry`](../tolineargeometry) especificación.
