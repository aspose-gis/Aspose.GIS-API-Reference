---
title: ShapefileDriver.OpenLayer
second_title: Αναφορά Aspose.GIS για .NET API
description: ShapefileDriver μέθοδος. Ανοίγει ένα επίπεδο για ανάγνωση.
type: docs
weight: 60
url: /el/net/aspose.gis.formats.shapefile/shapefiledriver/openlayer/
---
## OpenLayer(string, ShapefileOptions) {#openlayer_5}

Ανοίγει ένα επίπεδο για ανάγνωση.

```csharp
public VectorLayer OpenLayer(string path, ShapefileOptions options)
```

| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| path | String | Διαδρομή προς το αρχείο. |
| options | ShapefileOptions | Επιλογές για συγκεκριμένα προγράμματα οδήγησης. |

### Επιστρεφόμενη Αξία

Ένα παράδειγμα του[`VectorLayer`](../../../aspose.gis/vectorlayer/).

### Δείτε επίσης

* class [VectorLayer](../../../aspose.gis/vectorlayer/)
* class [ShapefileOptions](../../shapefileoptions/)
* class [ShapefileDriver](../)
* χώρος ονομάτων [Aspose.Gis.Formats.Shapefile](../../shapefiledriver/)
* συνέλευση [Aspose.GIS](../../../)

---

## OpenLayer(AbstractPath, DriverOptions) {#openlayer_1}

Ανοίγει ένα επίπεδο για ανάγνωση.

```csharp
public override VectorLayer OpenLayer(AbstractPath path, DriverOptions options)
```

| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| path | AbstractPath | Διαδρομή προς το αρχείο. |
| options | DriverOptions | Επιλογές για συγκεκριμένα προγράμματα οδήγησης. |

### Επιστρεφόμενη Αξία

Ένα παράδειγμα του[`VectorLayer`](../../../aspose.gis/vectorlayer/).

### Δείτε επίσης

* class [VectorLayer](../../../aspose.gis/vectorlayer/)
* class [AbstractPath](../../../aspose.gis/abstractpath/)
* class [DriverOptions](../../../aspose.gis/driveroptions/)
* class [ShapefileDriver](../)
* χώρος ονομάτων [Aspose.Gis.Formats.Shapefile](../../shapefiledriver/)
* συνέλευση [Aspose.GIS](../../../)

---

## OpenLayer(AbstractPath, ShapefileOptions) {#openlayer_2}

Ανοίγει ένα επίπεδο για ανάγνωση.

```csharp
public VectorLayer OpenLayer(AbstractPath path, ShapefileOptions options)
```

| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| path | AbstractPath | Διαδρομή προς το αρχείο. |
| options | ShapefileOptions | Επιλογές για συγκεκριμένα προγράμματα οδήγησης. |

### Επιστρεφόμενη Αξία

Ένα παράδειγμα του[`VectorLayer`](../../../aspose.gis/vectorlayer/).

### Δείτε επίσης

* class [VectorLayer](../../../aspose.gis/vectorlayer/)
* class [AbstractPath](../../../aspose.gis/abstractpath/)
* class [ShapefileOptions](../../shapefileoptions/)
* class [ShapefileDriver](../)
* χώρος ονομάτων [Aspose.Gis.Formats.Shapefile](../../shapefiledriver/)
* συνέλευση [Aspose.GIS](../../../)


