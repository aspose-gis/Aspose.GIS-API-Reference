...
description: Map methode. Erstellt einVectorMapLayer mit dem Standardsymbolisierer und fügt ihn der Karte hinzu. Ebenen werden in zusätzlicher Reihenfolge gerendert.
type: docs
weight: 110
...
| keepOpen | Boolean | `true` um die Vektorebene nach dem geöffnet zu lassen[`Map`](../) Objekt wird entsorgt; `false` um die Schicht zu entsorgen. |
...
| keepOpen | Boolean | `true` um die Ebene nach dem offen zu lassen[`Map`](../) Objekt wird entsorgt; ansonsten,`false` . |
...
| keepOpen | Boolean | `true` um die Rasterebene nach dem offen zu lassen[`Map`](../) Objekt wird entsorgt; `false` um die Schicht zu entsorgen. |
