---
title: FeatureBasedConfiguration
second_title: Aspose.GIS لمرجع .NET API
description: رد اتصال يتم استخدامه لتكوين هذه التسمية قبل التعامل مع ميزة.
type: docs
weight: 20
url: /ar/net/aspose.gis.rendering.labelings/simplelabeling/featurebasedconfiguration/
---
## SimpleLabeling.FeatureBasedConfiguration property

رد اتصال يتم استخدامه لتكوين هذه التسمية قبل التعامل مع ميزة.

```csharp
public Action<Feature, SimpleLabeling> FeatureBasedConfiguration { get; set; }
```

### ملاحظات

يتم استدعاء رد الاتصال هذا قبل تسمية كل ميزة. إنه يقبل ميزة على وشك أن يتم تسميتها ونسخة من هذه التسمية. من خلال تغيير خصائص الاستنساخ ، من الممكن تحديث سلوك الملصقات استنادًا إلى سمات الميزة.

### أنظر أيضا

* class [Feature](../../../aspose.gis/feature)
* class [SimpleLabeling](../../simplelabeling)
* مساحة الاسم [Aspose.Gis.Rendering.Labelings](../../simplelabeling)
* المجسم [Aspose.GIS](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.GIS.dll -->
