---
title: "WarpOptions.TargetSpatialReferenceSystem"
second_title: "مرجع API لـ Aspose.GIS لـ .NET"
description: "خاصية WarpOptions. تحدد المرجع المكاني المستهدف. إذا تم تعيينه إلى null يتم استخدام المرجع الافتراضي أو مرجع المصدر"
type: docs
weight: 70
url: /ar/net/aspose.gis.raster/warpoptions/targetspatialreferencesystem/
---
## WarpOptions.TargetSpatialReferenceSystem property

يحدد المرجع المكاني المستهدف. إذا تم تعيينه إلى `null`، يُستخدم المرجع الافتراضي أو المرجع المصدر.

```csharp
public SpatialReferenceSystem TargetSpatialReferenceSystem { get; set; }
```

### انظر أيضًا

* class [SpatialReferenceSystem](../../../aspose.gis.spatialreferencing/spatialreferencesystem/)
* class [WarpOptions](../)
* namespace [Aspose.Gis.Raster](../../warpoptions/)
* assembly [Aspose.GIS](../../../)


