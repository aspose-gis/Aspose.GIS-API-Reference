---
title: "OsmXmlOptions クラス"
type: docs
weight: 20
url: /ja/python-net/aspose.gis.formats.osmxml/osmxmloptions/
---
...
| **Name** | **説明** |
| :- | :- |
| [OsmXmlOptions()](#OsmXmlOptions__1) | 新しいインスタンスを作成します。 |
## **Properties**
| **Name** | **Type** | **Access** | **説明** |
| :- | :- | :- | :- |
...
| report_all_nodes | bool | 読み/書き | 重要なタグがなくても、すべてのノードをフィーチャーとして報告します。 |
| report_all_ways | bool | 読み/書き | 重要なタグがなくても、またはノードがなくても、すべてのウェイをフィーチャーとして報告します。 |
| report_common_attributes | bool | 読み/書き | 共通の OSM 属性（visible、version、changeset、timestamp、user、uid）を報告します。<br/>            共通属性は "osm_" プレフィックス付きのフィーチャ属性として報告されます。例: osm_user、osm_timestamp など。 |
...
| write_common_attributes | bool | 読み/書き | 共通の OSM 属性（visible、version、changeset、timestamp、user、uid）を書き込みます。 |
