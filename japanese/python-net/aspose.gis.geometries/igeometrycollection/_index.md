---
title: "IGeometryCollection クラス"
type: docs
weight: 120
url: /ja/python-net/aspose.gis.geometries/igeometrycollection/
---
...
| [get_length()](#get_length__21) | このジオメトリの長さを計算します。 |
| [get_point_on_surface()](#get_point_on_surface__22) | このコレクション内のいずれかのサーフェス上に必ず存在する点を検索します。 |
| [intersection(other)](#intersection_other_23) | このジオメトリと指定されたジオメトリの交差を構築します。 |
| [intersects(extent)](#intersects_extent_24) | このジオメトリが指定された範囲と交差するかどうかを判定します。 |
| [intersects(other)](#intersects_other_25) | このジオメトリと指定されたジオメトリが交差するかどうかを判定します。 |
| [overlaps(other)](#overlaps_other_26) | このジオメトリが指定されたジオメトリと重なるかどうかを判定します。 |
| [relate(other, intersection_pattern_matrix)](#relate_other_intersection_pattern_matrix_27) | このジオメトリと指定されたジオメトリの DE-9IM 交差行列が提供されたパターンと一致するかどうかを判定します。 |
| [replace_polygons_by_lines()](#replace_polygons_by_lines__28) | このジオメトリのラインとして表現されたポリゴンを取得します。 |
| [spatially_contains(other)](#spatially_contains_other_29) | このジオメトリが空間的に指定されたジオメトリを含むかどうかを判定します。 |
| [spatially_equals(other)](#spatially_equals_other_30) | このジオメトリが空間的に指定されたジオメトリと等しいかどうかを判定します。 |
| [sym_difference(other)](#sym_difference_other_31) | このジオメトリと指定されたジオメトリの対称差を構築します。 |
| [to_editable()](#to_editable__32) | このジオメトリの編集可能なコピーを取得します。 |
| [to_linear_geometry()](#to_linear_geometry__33) | デフォルトの<c>tolerance</c>を使用して、このジオメトリの近似または同等の非曲線バージョンを取得します。 |
| [to_linear_geometry(tolerance)](#to_linear_geometry_tolerance_34) | 指定された<c>tolerance</c>を使用して、このジオメトリの近似または同等の非曲線バージョンを取得します。 |
| [touches(other)](#touches_other_35) | このジオメトリと指定されたジオメトリが接触しているかどうかを判定します。 |
| [union(other)](#union_other_36) | このジオメトリと指定されたジオメトリを結合します。 |
| [union(other)](#union_other_37) | このジオメトリと指定されたジオメトリを結合します。 |
| [within(extent)](#within_extent_38) | このジオメトリが指定された範囲内にあるかどうかを判定します。 |
| [within(other)](#within_other_39) | このジオメトリが指定されたジオメトリ内にあるかどうかを判定します。 |
...
| 型 | 説明 |
| :- | :- |
| [GeometryCollection](/psd/python-net/aspose.gis.geometries/geometrycollection) | このジオメトリの編集可能なコピー。 |
...
| 型 | 説明 |
| :- | :- |
| [IGeometryCollection](/psd/python-net/aspose.gis.geometries/igeometrycollection) | 曲線ジオメトリを持たないジオメトリです。これは <DOM Element: class at 0x2a1791cc040>.IGeometryCollection.to_linear_geometry()(float) をデフォルトの <c>tolerance</c> で使用した場合と同等です。デフォルトの <c>tolerance</c> の値はこのジオメトリの [SpatialReferenceSystem](/psd/python-net/aspose.gis.spatialreferencing/spatialreferencesystem/) の影響を受けます:<br/> <ul><br/> <li> 投影座標系の場合、許容誤差は 0.001 メートル（SRS の単位）です </li><br/> <li> 地理座標系の場合、許容誤差は <c>1e-5</c> 度（SRS の単位）です </li><br/> <li> 不明な座標系の場合、許容誤差は <c>1e-5</c> です </li><br/> </ul><br/> 適用される変換の詳細については、<DOM Element: class at 0x2a1791cc040>.IGeometryCollection.to_linear_geometry()(float) の仕様を参照してください。 |
