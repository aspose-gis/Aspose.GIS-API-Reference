---
title: "CompoundSpatialReferenceSystem クラス"
type: docs
weight: 30
url: /ja/python-net/aspose.gis.spatialreferencing/compoundspatialreferencesystem/
---
...
| **Name** | **Type** | **Access** | **説明** |
| :- | :- | :- | :- |
| as_compound | [CompoundSpatialReferenceSystem](/psd/python-net/aspose.gis.spatialreferencing/compoundspatialreferencesystem) | r | これを返します。 |
...
| as_geographic | [GeographicSpatialReferenceSystem](/psd/python-net/aspose.gis.spatialreferencing/geographicspatialreferencesystem) | r | この SRS の地理的表現を返します。もしこの複合 SRS が実際に地理的 SRS を表す場合、結果は<br/>            3 次元の地理的 SRS（経度、緯度、高さの次元を含む）になります。そうでない場合は例外がスローされます。 |
...
| as_projected | [ProjectedSpatialReferenceSystem](/psd/python-net/aspose.gis.spatialreferencing/projectedspatialreferencesystem) | r | この SRS の投影表現を返します。この複合 SRS が実際に投影 SRS を表す場合、結果は<br/>            3 次元の投影 SRS（X、Y、高さの次元を含む）になります。そうでない場合は例外がスローされます。 |
...
| dimensions_count | int | r | 次元数。複合 SRS の場合、これは基礎となる SRS の次元数の合計です。 |
...
| geographic_datum | [GeographicDatum](/psd/python-net/aspose.gis.spatialreferencing/geographicdatum) | r | この SRS の測地基準を返します。<br/>            両方の [CompoundSpatialReferenceSystem.head](/psd/python-net/aspose.gis.spatialreferencing/compoundspatialreferencesystem/) と [CompoundSpatialReferenceSystem.tail](/psd/python-net/aspose.gis.spatialreferencing/compoundspatialreferencesystem/) が測地基準を持つ場合は、head の測地基準を返します。 |
| has_geographic_datum | bool | r | 基礎となる SRS のいずれかが測地基準を持っている場合、複合 SRS は測地基準を持ちます。 |
| has_prime_meridian | bool | r | 基礎となる SRS のいずれかが本初子午線を持っている場合、複合 SRS は本初子午線を持ちます。 |
| head | [SpatialReferenceSystem](/psd/python-net/aspose.gis.spatialreferencing/spatialreferencesystem) | r | 最初の基礎 SRS。 |
| identifier | [Identifier](/psd/python-net/aspose.gis.spatialreferencing/identifier) | r | この識別可能オブジェクトの識別子。 |
| is_compound | bool | r | <see langword=\"true\" /> を返します。 |
...
| prime_meridian | [PrimeMeridian](/psd/python-net/aspose.gis.spatialreferencing/primemeridian) | r | この SRS の本初子午線を返します。<br/>            両方の [CompoundSpatialReferenceSystem.head](/psd/python-net/aspose.gis.spatialreferencing/compoundspatialreferencesystem/) と [CompoundSpatialReferenceSystem.tail](/psd/python-net/aspose.gis.spatialreferencing/compoundspatialreferencesystem/) が本初子午線を持つ場合は、head の本初子午線を返します。 |
| tail | [SpatialReferenceSystem](/psd/python-net/aspose.gis.spatialreferencing/spatialreferencesystem) | r | 2 番目の基礎 SRS。 |
| type | [SpatialReferenceSystemType](/psd/python-net/aspose.gis.spatialreferencing/spatialreferencesystemtype) | r | この複合 SRS のタイプ。測地系と垂直系の組み合わせの場合は [SpatialReferenceSystemType.GEOGRAPHIC](/psd/python-net/aspose.gis.spatialreferencing/spatialreferencesystemtype/) になります。<br/>            この複合 SRS が投影系と垂直系の組み合わせの場合は [SpatialReferenceSystemType.PROJECTED](/psd/python-net/aspose.gis.spatialreferencing/spatialreferencesystemtype/) になります。 |
...
| [validate(error_message)](#validate_error_message_18) | この SRS が有効かどうかを判断します。有効性の説明については <see cref=\"M:Aspose.Gis.SpatialReferencing.SpatialReferenceSystem.Validate(string@)\" /> を参照してください。 |
...
この SRS が有効かどうかを判断します。有効性の説明については <see cref=\"M:Aspose.Gis.SpatialReferencing.SpatialReferenceSystem.Validate(string@)\" /> を参照してください。

**Parameters:**

| パラメーター | 型 | 説明 |
| :- | :- | :- |
| error_message | 文字列 | 無効性の説明（結果が <see langword="false" /> の場合） |

**Returns**

| 型 | 説明 |
| :- | :- |
| bool | この SRS が有効な場合は - <see langword="true" />、それ以外の場合は - <see langword="false" />。 |
