---
title: LocalSpatialReferenceSystem.HasGeographicDatum
second_title: Aspose.GIS for .NET API リファレンス
description: LocalSpatialReferenceSystem 財産. 戻り値falseローカル SRS には地理データがありません
type: docs
weight: 40
url: /ja/net/aspose.gis.spatialreferencing/localspatialreferencesystem/hasgeographicdatum/
---
## LocalSpatialReferenceSystem.HasGeographicDatum property

戻り値`false`、ローカル SRS には地理データがありません。

```csharp
public override bool HasGeographicDatum { get; }
```

### 関連項目

* class [LocalSpatialReferenceSystem](../)
* 名前空間 [Aspose.Gis.SpatialReferencing](../../localspatialreferencesystem/)
* 組み立て [Aspose.GIS](../../../)


